...
# Pre-requisites:
## Download raw data and data key (can be done with `drive-interactions / 00_download.R`)
...
# Identify file name
clim_rawname <- "00_2024_LTER Climate_Survey_11_19_2024_11.16_de_identified.xlsx"
...
clim_list <- list()
clim_list[[gsub("00_", "", clim_rawname)]] <- clim_v1
...
# Read in data key
key_obj <- readxl::read_excel(path = file.path("data", "raw", "00_climate24_data-key.xlsx")) %>% 
...
clim_v2 <- ltertools::standardize(focal_file = gsub("00_", "", clim_rawname),
...
clim_v6 <- clim_v5 %>% 
  ...
  dplyr::mutate(site = ifelse(site == "Other",
    ...
    yes = "Plum Island Ecosystems LTER (PIE)", no = site)) %>% 
  # Split into separate columns by parentheses
  tidyr::separate_wider_delim(cols = site, delim = " (", names = c("site_name", "site")) %>% 
...
write.csv(x = clim_v99, row.names = F, na = '',
  file = file.path("data", "climate-01_processed-data.csv"))
